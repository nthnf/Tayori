...
use crate::backend::models::moonshine::Quantization;
use crate::backend::models::moonshine::decode::GreedyDecoder;
use crate::backend::models::moonshine::session;
use crate::backend::models::moonshine::{
...
impl BinTokenizer {
    ...
    pub fn decode(&self, tokens: &[i64]) -> Result<String, TranscribeError> {
...
impl StreamingModel {
    pub fn new_state(&self) -> StreamingState {
        StreamingState::new(&self.config)
    }

    pub fn reset_state(&self, state: &mut StreamingState) {
        state.reset(&self.config);
    }

    pub fn decode_current_state(
        &mut self,
        state: &mut StreamingState,
        is_final: bool,
    ) -> Result<String, TranscribeError> {
        self.encode_streaming(state, is_final)?;
        ...
        // Dynamic sequence length to prevent unbounded hallucination loops.
        // Assumes max ~6 tokens per second of audio (frame_count = 10ms frames).
        let duration_sec = state.frame_count as f32 / 100.0;
        let dynamic_max = ((duration_sec * 6.0).ceil() as usize).max(20);
        let max_tokens = self.config.max_seq_len.min(dynamic_max);

        let mut greedy =
            crate::backend::models::moonshine::decode::GreedyDecoder::new(self.config.eos_id);
        let mut tokens: Vec<i64> = Vec::new();
        let mut current_token = self.config.bos_id;

        for _ in 0..max_tokens {
            let logits = self.decode_step_logits(state, current_token)?;
    ...
    pub fn process_audio_chunk(
        ...
        let audio_view = ArrayViewD::from_shape(IxDyn(&[1, chunk_len]), audio_chunk)?;
        let sample_buffer_view = ArrayViewD::from_shape(IxDyn(&[1, 79]), &state.sample_buffer)?;
        let sample_len_val = [state.sample_len];
        let sample_len_view = ArrayViewD::from_shape(IxDyn(&[1]), &sample_len_val)?;
        let conv1_view = ArrayViewD::from_shape(
            IxDyn(&[1, self.config.d_model_frontend, 4]),
            &state.conv1_buffer,
        )?;
        let conv2_view =
            ArrayViewD::from_shape(IxDyn(&[1, self.config.c1, 4]), &state.conv2_buffer)?;
        let frame_count_val = [state.frame_count];
        let frame_count_view = ArrayViewD::from_shape(IxDyn(&[1]), &frame_count_val)?;

        let t_audio_chunk = TensorRef::from_array_view(audio_view)?;
        let t_sample_buffer = TensorRef::from_array_view(sample_buffer_view)?;
        let t_sample_len = TensorRef::from_array_view(sample_len_view)?;
        let t_conv1_buffer = TensorRef::from_array_view(conv1_view)?;
        let t_conv2_buffer = TensorRef::from_array_view(conv2_view)?;
        let t_frame_count = TensorRef::from_array_view(frame_count_view)?;
        ...
        let sample_buffer_out = outputs
            ...
            .try_extract_array::<f32>()?;
        let sample_buffer_data = sample_buffer_out.as_slice().ok_or_else(|| {
            TranscribeError::Inference("sample_buffer_out not contiguous".to_string())
        })?;
        state.sample_buffer = sample_buffer_data[..79].to_vec();
        ...
        let sample_len_data = sample_len_out.as_slice().ok_or_else(|| {
            TranscribeError::Inference("sample_len_out not contiguous".to_string())
        })?;
        state.sample_len = sample_len_data[0];
        ...
        let conv1_out = outputs
            ...
            .try_extract_array::<f32>()?;
        let conv1_data = conv1_out.as_slice().ok_or_else(|| {
            TranscribeError::Inference("conv1_buffer_out not contiguous".to_string())
        })?;
        ...
        let conv2_out = outputs
            ...
            .try_extract_array::<f32>()?;
        let conv2_data = conv2_out.as_slice().ok_or_else(|| {
            TranscribeError::Inference("conv2_buffer_out not contiguous".to_string())
        })?;
        ...
        let frame_count_out = outputs
            ...
            .try_extract_array::<i64>()?;
        let frame_count_data = frame_count_out.as_slice().ok_or_else(|| {
            TranscribeError::Inference("frame_count_out not contiguous".to_string())
        })?;
        state.frame_count = frame_count_data[0];
    ...
    pub fn compute_cross_kv(&mut self, state: &mut StreamingState) -> Result<(), TranscribeError> {
        if state.memory_len == 0 {
        ...
        let k_cross_data = k_cross
            .as_slice()
            .ok_or_else(|| TranscribeError::Inference("k_cross not contiguous".to_string()))?;
        let v_cross_data = v_cross
            .as_slice()
            .ok_or_else(|| TranscribeError::Inference("v_cross not contiguous".to_string()))?;

        state.k_cross = k_cross_data[..kv_size].to_vec();
        state.v_cross = v_cross_data[..kv_size].to_vec();
    ...
    pub fn run_decoder(
        ...
        let k_self_data = k_self_out
            .as_slice()
            .ok_or_else(|| TranscribeError::Inference("out_k_self not contiguous".to_string()))?;
        let v_self_data = v_self_out
            .as_slice()
            .ok_or_else(|| TranscribeError::Inference("out_v_self not contiguous".to_string()))?;

        let k_src = &k_self_data[..new_cache_size];
        let v_src = &v_self_data[..new_cache_size];
...
impl SpeechModel for StreamingModel {
    fn transcribe_raw(
        ...
        unimplemented!(
            "StreamingModel should be used with process_audio_chunk and decode_current_state, not transcribe_raw"
        )
    }
}
