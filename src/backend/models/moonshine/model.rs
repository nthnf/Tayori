...
use crate::backend::models::moonshine::decode::{GreedyDecoder, parse_byte_token};
...
impl MoonshineTokenizer {
    fn new(model_dir: &Path) -> Result<Self, TranscribeError> {
        ...
        if let Some(model) = json.get("model")
            && let Some(v) = model.get("vocab").and_then(|v| v.as_object())
        {
            for (token, id) in v {
        ...
        if let Some(added_tokens) = json.get("added_tokens").and_then(|v| v.as_array()) {
            for token in added_tokens {
                let is_special = token
                    ...
                    .unwrap_or(false);
                if is_special && let Some(id) = token.get("id").and_then(|v| v.as_u64()) {
