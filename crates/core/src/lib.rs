...
///
/// There is a small adapter thread between VAD and STT because VAD owns audio
/// terms (`SpeechSegment`) while STT owns transcription terms (`TranscriptionJob`).
/// The adapter is intentionally simple: receive segment, convert it, send job.
/// It drains queued segments with `try_recv` to avoid adding a 10 ms poll delay
/// per segment when VAD emits more than one segment close together.
///
/// Latency checkpoints to inspect with `RUST_LOG=debug`:
///
/// - capture metrics: device callback health and raw ring drops.
/// - resample metrics: input/output audio processed and VAD ring drops.
/// - VAD start/end logs: silence-boundary latency and emitted segment length.
/// - adapter logs: segment-to-job forwarding latency.
/// - Whisper metrics: inference time for each segment.
///
/// The segment and job channels are currently unbounded. This avoids blocking
/// the audio callback, but it also means backlog can grow if downstream stages
/// stop consuming or Whisper is slower than real time.
...
fn forward_segment(
    ...
    let sent = job_tx
        .send(TranscriptionJob::new(segment.index, segment.samples))
        .is_ok();
